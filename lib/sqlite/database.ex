...
def handle_info({:DOWN, _ref, :process, _pid, :normal}, state),
  ...
  def insert_data(start_count, end_count) do

     count = end_count -  start_count
     start_count..end_count
      |> Stream.map(fn counter -> Task.async(fn -> GenServer.call( String.to_atom("user#{counter}"), {:add_record,   String.to_atom("user#{counter}"),counter}) end)
      # [max_concurrency: count, timeout: 30_000]) # Added the timeout here
      end)
      |> Enum.map(&Task.await(&1))
      # |> Task.await_many()
  end
  def handle_call({:add_record,name,count}, _from, state) do

    # Task.async(fn ->
      disk_database = Map.fetch!(state, :disk_conn)
      memory_database = Map.fetch!(state, :memory_conn)
      insert(100,disk_database,name,count)
      insert(100,memory_database,name,count)
    # end)
    # |> Task.await()
    # Task.Supervisor.async_nolink(Sqlite.DatabaseSupervisor, fn ->
